...
function RecipeForm({ lang, setLang, recipe, onSave, onCancel, saving, showToast, C, S }) {
  ...
  const handleImage = async (e) => {
    ...
    try {
      ...
      if (isHeic) {
        showToast(lang === "de" ? "Konvertiere HEIC…" : "Converting HEIC…");
...
//  THEMES & STYLES
// ═════════════════════════════════════════════
const LIGHT = { warm: "#4A3228", cream: "#FDF6EC", parchment: "#F0E0C8", accent: "#B8532E", accentLight: "#D4835F", soft: "#8B7262", card: "#FFFBF5", border: "#DFC9AF", red: "#D32F2F", bg1: "#FDF6EC", bg2: "#F0E0C8", fieldBg: "#fff", text: "#4A3228" };
const DARK = { warm: "#E8D5C0", cream: "#1F1814", parchment: "#2A2019", accent: "#E88B6B", accentLight: "#F0A88D", soft: "#9B8676", card: "#2D231C", border: "#4A3A2C", red: "#FF6B6B", bg1: "#1A140F", bg2: "#2A2019", fieldBg: "#2D231C", text: "#E8D5C0" };
...
function makeStyles(C) {
  return {
    app: { fontFamily: body, background: `linear-gradient(178deg, ${C.bg1} 0%, ${C.bg2} 50%, ${C.bg1} 100%)`, minHeight: "100vh", color: C.text, maxWidth: 1040, margin: "0 auto", padding: "0 20px 48px", position: "relative" },
    ...
    overlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 900, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 },
    modal: { background: C.card, borderRadius: 18, padding: 28, maxWidth: 380, width: "100%", boxShadow: `0 20px 60px rgba(0,0,0,0.4)` },
    ...
    logo: { fontSize: 44 },
    title: { fontFamily: font, fontSize: 28, margin: 0, fontWeight: 800, letterSpacing: "-0.03em", color: C.warm },
    subtitle: { margin: "2px 0 0", fontSize: 13, color: C.soft, fontStyle: "italic", fontFamily: body },
    headerRight: { display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" },
    addBtn: { background: C.accent, color: "#fff", border: "none", borderRadius: 12, padding: "10px 16px", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: body },
    ...
    langToggle: { background: `${C.accent}12`, border: `1.5px solid ${C.accent}35`, borderRadius: 12, padding: "8px 14px", cursor: "pointer", fontSize: 14, fontWeight: 600, color: C.accent, fontFamily: body },

    sectionTabs: { display: "flex", gap: 6, marginTop: 16, padding: 4, background: C.card, borderRadius: 14, border: `1px solid ${C.border}`, flexWrap: "wrap" },
    sectionTab: { flex: "1 1 auto", minWidth: 140, padding: "10px 16px", borderRadius: 10, border: "none", background: "transparent", cursor: "pointer", fontSize: 14, fontWeight: 600, color: C.soft, fontFamily: body, transition: "all 0.2s", position: "relative" },
    sectionTabActive: { background: C.accent, color: "#fff" },

    userGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 14 },
    userCard: { background: C.card, borderRadius: 14, padding: 16, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 14, cursor: "pointer", transition: "all 0.2s" },
    userAvatar: { width: 50, height: 50, borderRadius: "50%", background: C.accent, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 700, fontFamily: font, flexShrink: 0 },
    userName: { fontFamily: font, fontSize: 18, color: C.warm, margin: 0, fontWeight: 700 },
    userMeta: { color: C.soft, fontSize: 13, margin: "4px 0 0" },

    toolbar: { marginTop: 16 },
    searchWrap: { position: "relative", display: "flex", alignItems: "center", background: C.fieldBg, borderRadius: 14, border: `1.5px solid ${C.border}`, padding: "0 16px" },
    ...
    catChip: { background: "transparent", border: `1.5px solid ${C.border}`, borderRadius: 20, padding: "7px 14px", fontSize: 13, cursor: "pointer", color: C.soft, fontWeight: 600, fontFamily: body, whiteSpace: "nowrap" },
    catChipActive: { background: C.accent, color: "#fff", borderColor: C.accent },
    favToggle: { background: "transparent", border: `1.5px solid ${C.border}`, borderRadius: 20, padding: "7px 14px", fontSize: 13, cursor: "pointer", color: C.soft, fontWeight: 600, fontFamily: body },
    ...
    cardFav: { position: "absolute", top: 10, right: 10, background: "rgba(255,255,255,0.85)", border: "none", borderRadius: "50%", width: 36, height: 36, cursor: "pointer", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" },
    ...
    cardOwner: { fontSize: 12, color: C.accent, margin: "0 0 6px", fontWeight: 600 },
    ...
    portionBtn: { width: 42, height: 42, borderRadius: "50%", border: `2.5px solid ${C.accent}`, background: "transparent", color: C.accent, fontSize: 22, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: body },
    ...
    stepNum: { width: 36, height: 36, borderRadius: "50%", background: C.accent, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 15, flexShrink: 0, fontFamily: font },
    stepText: { margin: 0, lineHeight: 1.7, fontSize: 15, color: C.text, paddingTop: 6 },
    ...
    uploadPlaceholder: { width: "100%", maxWidth: 400, height: 160, borderRadius: 16, border: `2px dashed ${C.border}`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", background: `${C.parchment}60` },
    uploadText: { fontSize: 14, color: C.soft, marginTop: 8 },
    emojiBtn: { width: 40, height: 40, borderRadius: 10, border: `1px solid ${C.border}`, background: C.fieldBg, fontSize: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" },
    ...
    textarea: { width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 15, fontFamily: body, color: C.text, background: C.fieldBg, resize: "vertical" },
    ...
    addRowBtn: { background: `${C.accent}15`, border: `1.5px dashed ${C.accent}50`, borderRadius: 10, padding: "10px 20px", cursor: "pointer", fontSize: 14, color: C.accent, fontWeight: 600, fontFamily: body, marginTop: 8, width: "100%" },
    ...
    saveBtn: { padding: "12px 32px", borderRadius: 12, border: "none", background: C.accent, color: "#fff", cursor: "pointer", fontSize: 15, fontWeight: 700, fontFamily: body, boxShadow: `0 4px 16px ${C.accent}50` },
  };
}  const [recipes, setRecipes] = useState([]);
  const [favorites, setFavorites] = useState([]);
  const [loading, setLoading] = useState(true);
